...
If you use typescript (a good option), the `ts/` folder will be compiled to the output directory's `/js` folder in the same way as sass. You're expected to install `tsc` normally.
There is an option (in the `build_tools/toolchain` section referenced in #installation) to install and use webpack.
